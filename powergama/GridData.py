...
class GridData(object):
    ...
    def read_csv_files(self, file_input, dataset):
        ...
        if isinstance(file_input, str) or isinstance(file_input, pathlib.Path):
            # Single file - try with index_col=0 first to handle extra index column
            try:
                df = pd.read_csv(file_input, dtype=dtype, index_col=0)
                df = df.reset_index(drop=True)  # Reset to standard integer index
            except:
                # Fallback to reading without index_col if it fails
            ...
            for file in file_input:
                try:
                    temp_df = pd.read_csv(file, dtype=dtype, index_col=0)
                    temp_df = temp_df.reset_index(drop=True)
                    df_list.append(temp_df)
                except:
                    df_list.append(pd.read_csv(file, dtype=dtype))
