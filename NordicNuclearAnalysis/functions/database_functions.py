...
def getStorageFillingInAreaFromDB(data: GridData, db: Database, areas, generator_type, relative_storage, timeMaxMin):
    ...
    # Query the database - returns list of summed storage values per timestep
    storage_values = db.getResultStorageFillingMultiple(generators, timeMaxMin)

    # Process results - values are indexed from 0 to len-1, corresponding to timeMaxMin range
    filling = {}
    start_timestep = timeMaxMin[0]
    for i, storage in enumerate(storage_values):
