...
def plotEnergyMix(
        ...
        annual=False, years=30, ylim=None, savefig=None, title_suffix=""
):
    """
    ...
    annual : boolean
        If True, divide by number of years to show annual values
    years : int
        Number of years to divide by when annual=True (default 30)
    ylim : tuple or None
        If provided, set fixed y-axis limits (e.g., (0, 250))
    ...
    dfplot_twh = dfplot.loc[areas, gentypes] / 1e6

    # Convert to annual if requested
    if annual:
        dfplot_twh = dfplot_twh / years
    ...
    dfplot_twh.index = [country_names.get(a, a) for a in dfplot_twh.index]

    ax = dfplot_twh.plot(kind="bar", stacked=True, color=colors)

    # Set axis labels
    if annual:
        plt.ylabel("Annual production (TWh/year)")
    else:
        plt.ylabel("TWh")
    if len(areas) == 1:
        plt.xlabel("")  # Remove x-label if only one country
        ax.set_xticklabels([])  # Remove x-tick labels for single country
...
def plotZonePriceMatrix(price_matrix, plot_config):
    ...
    plt.xlabel(plot_config.get('x_label', 'Year'))
    plt.ylabel("Zone")
